...
- **classify response shapes:** block → adds `reason: "prompt_injection_detected"`; flag → adds `elevated_scrutiny: True`; always includes `content_source_mismatch: bool`. The optional `session_id: str = ""` parameter on `classify` controls session tagging (empty string disables it).
