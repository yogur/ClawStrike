...
- **MCP server pattern:** `src/clawstrike/mcpserver.py` holds a module-level `mcp = FastMCP(...)` instance with `_config: ClawStrikeConfig | None` and `_classifier: BaseClassifier | None` globals. Call `init_server(cfg)` to inject config and load the classifier before the server handles requests. Tests mock `create_classifier` via `patch("clawstrike.mcpserver.create_classifier", ...)` in the `autouse` fixture and reset both globals to `None` after each test.
